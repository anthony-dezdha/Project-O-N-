...
class Graph:
    ...
    def compute_depth(self):
         def dfs(v_index, path):
             #mark node as seen, like gray in knight_tour, dfs
             self.vertices[v_index].visited = True #suggested pylint
             path.append(self.vertices[v_index].label)
             siblings = self.get_adjacent_vertices(v_index)
             if len(siblings) == 0:
                 self.vertices[v_index].depth = 0
                 path.pop()
                 self.vertices[v_index].visited = False
                 return 0

             maxdep = 0
             d = 0
             while d < len(siblings):
                 s = siblings[d]
                 if not self.vertices[s].visited:
                     sigdepth = dfs(s, path)
                     if sigdepth + 1 > maxdep:
                         maxdep = sigdepth + 1
                 d += 1
             self.vertices[v_index].depth = maxdep
             path.pop()
             self.vertices[v_index].visited = False
             return maxdep
    ...
    def get_registration_plan(self):
        ...
        courses = []
        finished = []
        logged = len(self.vertices)
        while len(finished ) < logged:
            semlst = []

            for i in range(logged):
                if self.vertices[i].label in finished:
                    continue

                prereqsatisfied = True
                for p in range(logged):
                    if self.adjacency_matrix[p][i] == 1:
                        #p is a prereq for i
                        if self.vertices[p].label not in finished:
                            prereqsatisfied = False
                            break #non no not yet bud
                #if prereqsatisfied: like actually, add ASAP
                if prereqsatisfied:
                    semlst.append(self.vertices[i].label)
                    finished.append(self.vertices[i].label)
                    if len(semlst) == 4:
                        break
